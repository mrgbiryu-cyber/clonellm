...
const OUT_DIR = path.join(ROOT, "data", "visual", "plp");
const OUT_INDEX_PATH = path.join(OUT_DIR, "index.json");
const LOCK_PATH = path.join(ROOT, "tmp", "capture-plp.lock");
...
function randomPort() {
  return 10400 + Math.floor(Math.random() * 500);
}
...
async function launchChromeSession(viewport) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const userDataDir = fs.mkdtempSync(path.join(ROOT, "tmp", "cdp-plp-"));
    const chrome = spawn(
      CHROME,
      [
        "--headless",
...
async function captureVariant(session, target, sourceType, baseUrl) {
  const dir = path.join(OUT_DIR, target.pageId, target.viewportProfile);
  ensureDir(dir);
  ...
  await settlePlpPage(client);
  const extra = await captureArtifacts(client, screenshotPath, htmlPath, viewportFor(target.viewportProfile));
  ...
  const payload = {
    ...
    representativeCount: target.representativeCount || 0,
    representativeProducts: target.representativeProducts || [],
    fallbackFromViewportProfile: target.fallbackFromViewportProfile || null,
...
async function main() {
  ...
  const plpTargets = (targets.plpTargets || []).filter((item) => {
  ...
  const captures = [];
  const errors = [];
  for (const target of plpTargets) {
    let session = null;
    try {
      session = await launchChromeSession(viewportFor(target.viewportProfile));
      const capture = await captureVariant(session, target, sourceType, baseUrl);
      captures.push(capture);
      console.log(`captured ${sourceType} ${target.pageId} ${target.viewportProfile}`);
    } catch (error) {
      const failure = {
        sourceType,
        pageId: target.pageId,
      ...
      errors.push(failure);
      console.error(`capture failed ${sourceType} ${target.pageId} ${target.viewportProfile}: ${String(error)}`);
    } finally {
  ...
  for (const item of existingIndex.captures || []) {
    mergedMap.set(`${item.sourceType || 'reference'}:${item.pageId}:${item.viewportProfile}`, item);
  }
  for (const item of captures) {
    mergedMap.set(`${item.sourceType || 'reference'}:${item.pageId}:${item.viewportProfile}`, item);
  }
  const mergedCaptures = Array.from(mergedMap.values()).sort((a, b) => `${a.pageId}:${a.viewportProfile}:${a.sourceType}`.localeCompare(`${b.pageId}:${b.viewportProfile}:${b.sourceType}`));
