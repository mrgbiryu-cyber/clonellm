...
> 문서 상태:
> 이 문서는 `누적 상태 + 역사 기록 + 당시 판단`이 함께 들어 있는 대형 통합 문서다.
> 여기의 `TBD`, `placeholder`, 예시 코드 조각은 현재 production 기본값이 아니라 당시 검토 상태를 설명하는 경우가 많다.
...
> - `docs/document-placeholder-audit-2026-04-13.md`
...
| lower-content (브랜드쇼룸 등) | **source undecided at that time** (mobile 추정이었지만 당시 미검증) | historical note. 현재는 섹션별 재검증 결과를 별도 기준 문서에서 본다 |
...
placeholder-only 상태(빈 thumb box, line placeholder)를 오래 유지하면 텍스트 리듬 / 이미지 비율 / badge 위치가 계속 어긋난다. 따라서 실제 데이터 주입을 최대한 빨리 진행하는 것이 권장 패턴이다.
...
function renderBrandShowroomSection(items) {
  ...
  return `
    <section class="codex-brand-showroom"
             data-codex-slot="home-brand-showroom"
             data-source="undecided-at-review-time">
      <h2 class="codex-section-title">브랜드 쇼룸</h2>
...
// data-source: live reference 확인 후 결정 (pc / mo / undecided-at-review-time)
...
```
[ ] live reference screenshot 확인 (source: pc/mo/undecided-at-review-time 이때 결정)
...
| lower-content source undecided-at-review-time | 12.1 표, 12.2 Step3 | ✅ 수정 완료 |
...
### 12.5 실제 히스토리 기반 보정 — 이미 실패한 시도와 당시 우회 방식
...
즉 **skeleton 위치 + 별도 data source**가 당시 기준의 권장 패턴이었다.
...
즉 **placeholder-only phase는 짧게** 가져가는 것이 당시 기준의 권장 패턴이었다.
...
즉 **skeleton anchor + custom inner renderer**가 당시 기준의 반복 가능 패턴으로 정리됐다.
...
즉 이후 기준으로는 **home 하단 신규 섹션도 반드시 JS/CSS 원문 확인 후 작업**하는 방향으로 정리됐다.
...
```js
function renderBrandShowroomSection(items, { source = 'undecided-at-review-time' } = {}) {
  return `
    <section class="codex-brand-showroom"
             data-codex-slot="home-brand-showroom"
             data-source="${source}">
...
[ ] source를 pc/mo/undecided-at-review-time 중 하나로 기록했다
[ ] data-source에 undecided-at-review-time이 아닌 실제 확인값을 넣을 준비가 됐다
...
[ ] data-source에 실제 확인값을 붙였다 (pc / mo / undecided-at-review-time 금지)
[ ] 상대경로 이미지를 절대경로로 바꿨다
...
- 13.4 Step 2: `source: pc / mo / undecided-at-review-time` 명시, 성급한 단정 없음 ✅
- 13.4 Step 4의 renderer 함수 시그니처에 **문제 있음**:

```js
// 현재 코드
function renderBrandShowroomSection(items) {
  return `<section ... data-source="undecided-at-review-time">`;
}
```

`data-source="undecided-at-review-time"`가 함수 내부에 하드코딩되어 있다.
Step 2에서 source를 결정해도 renderer에 전달되는 경로가 없다.
결과적으로 실제 배포 후에도 `data-source="undecided-at-review-time"`가 HTML에 남는다.
...
체크리스트 항목 "data-source를 붙였다"도 실제로는 undecided-at-review-time이 붙은 상태를 점검하게 되므로, 아래로 교체해야 한다:
```
[ ] data-source에 undecided-at-review-time이 아닌 실제 확인된 값을 붙였다 (pc / mo)
```
...
| 항목 | 위치 | 수정 내용 | 심각도 |
|---|---|---|---|
| `data-source` 파라미터화 | 13.4 Step 4 | renderer 함수가 `source`를 파라미터로 받도록 수정 | **중** — source가 영구 undecided-at-review-time으로 남는 버그 |
...
- 섹션별 source를 `pc` / `mo`로 선언 금지 (모두 `undecided-at-review-time` 유지)
- `insertAfterAnchor(html, 'best-ranking', ...)` 같은 암묵 순서 가정 금지

이 규칙은 당시 임시 규칙이며, ground truth 문서가 생기면 해제 대상으로 본다.
...
#### 반박 C: source를 모두 `undecided-at-review-time`으로 되돌리면 현재 accepted baseline과 충돌한다
...
- 신규 섹션 또는 논쟁 중인 섹션만 `undecided-at-review-time` 허용
...
| C. source 전면 undecided-at-review-time 되돌리기는 코드-문서 충돌 | ✅ **수용** | `md-choice / timedeal / best-ranking / brand-showroom / latest-product-news`는 이미 태깅되어 있다. accepted-main 섹션의 태깅은 유지, 논쟁 중이거나 미검증 섹션만 `undecided-at-review-time`으로 둔다. |
...
function renderBrandShowroomSection(items, opts = {}) {
  const { useGroundTruth = false, source = 'undecided-at-review-time' } = opts;
...
[ ] data-codex-slot 유지
[ ] data-source가 undecided-at-review-time이 아닌 실제 확인된 값으로 교체됨
...
```html
data-codex-source="undecided-at-review-time"
```
...
- Updated home-lower diff snapshot: `smart-life 6.41%`, `subscription 4.69%`, `summary-banner-2 5.13%`, `space-renewal 4.72%`.
- `smart-life` was reduced further by syncing current live story cards and filling client-only image placeholders during capture.
- `subscription` and `summary-banner-2` remain at their stable baseline because forced image injection made the reference artifact less trustworthy.
- Remaining home-lower focus is now mainly acceptance verification rather than large structural correction.

## 23. Home Lower Cross-Review: Code-Level Findings

이 섹션은 `docs/home-lower-cross-review-checkpoint.md`에 대한 코드 레벨 크로스 리뷰 결과다.
리뷰 기준: `server.js`, `scripts/capture_home_lower_sections.mjs`, `data/normalized/plp-groups/index.json`.

### 23.1 Q1 — space-renewal 체감 이질감이 layout-specific issue인가

결론: **Yes. layout-specific issue는 맞지만, width rule 한 줄만으로는 닫히지 않았다.**

`server.js:7762` 근방에 아래 CSS rule이 존재한다.
...
동작:
1. 430px viewport 기준으로 `calc(100vw - 28px)` = **402px**
2. live에서 동일 구간은 **430px** 전폭으로 렌더됨
3. clone이 28px 좁게 렌더되므로 카드 3개의 inner text가 reflow됨
4. 결과적으로 section height가 live 대비 **+17px** delta 발생

이 가설은 "수치는 4.72%이지만 체감이 이질적인" 현상을 설명하는 1차 후보였다. 다만 `space-renewal`에 scoped width fix를 실제 runtime에 적용하고 재캡처한 뒤에도 height delta는 `+17px`로 유지됐다. 즉 width rule은 부분 원인일 수는 있어도, 현재 기준으로는 단독 원인으로 판정할 수 없다.

후속 측정으로 추가 확인된 점:
1. item count는 live/clone 모두 `3`으로 동일했다
2. 문제의 핵심은 `space-renewal`이 단일 카드 타입이 아니라 `1번째 hero + 2/3번째 compact row`의 mixed-card 섹션이라는 점이었다
3. clone은 generic replay CSS 때문에 모든 카드를 같은 세로형으로 강제하고 있었고, 이를 slot-scoped rule로 분리한 뒤 item geometry는 live 수준으로 맞췄다
4. 그 뒤에도 screenshot diff는 크게 튀었는데, 이유는 live reference 쪽 `img` DOM이 `visibility:hidden`, `naturalWidth=0` placeholder 상태로 잡히기 때문이다

즉 현재 `space-renewal`의 남은 리스크는 pure layout보다 **reference artifact 신뢰성**에 더 가깝다.

**수정 지시:**

```
server.js:7762
변경 전: width: calc(100vw - 28px) !important;
변경 후: width: 100% !important;
```

max-width: 430px는 유지해도 된다.

수정 후 검증:
1. `homeSandbox=space-renewal` 로 sandbox 재확인
2. `node scripts/capture_home_lower_sections.mjs` space-renewal 재실행
3. 결과: scoped width fix만으로는 height delta `17px` 유지
4. 이후 조치:
   - `list_inner` margin 복원
   - 첫 카드 hero layout / 2,3번째 compact row layout 분리
   - item geometry는 live 수준으로 정렬
5. 현재 단계:
   - screenshot diff는 live hidden placeholder image 때문에 inflated
   - manual visual acceptance 기준 병행 필요

### 23.2 Q2 — space-renewal: raw mobile replay 유지 vs custom renderer 재분리

결론: **raw mobile replay (mobile-derived) 유지가 맞다.**

이유:
1. `server.js:4412` `buildSpaceRenewalProducts()`는 `products.slice(0,3)` + `additionalMobileImageUrl` 우선 이미지 추출로 구현되어 있다.
2. `server.js:7762` `codex-home-space-renewal` custom renderer는 hero+grid 레이아웃이다. 이는 live의 horizontal card list와 구조가 완전히 다르다.
3. custom renderer로 분리하면 live의 카드 scrollable list 형태를 처음부터 다시 구현해야 하므로 오히려 더 많은 작업이 발생한다.
4. 현재 문제는 renderer 방식보다 inner card/grid/text rhythm 보정 문제에 가까웠고, 그 이후에는 live reference artifact 불안정이 더 큰 요인으로 드러났다.

**즉 mobile-derived 유지는 맞고, custom renderer 재분리는 불필요하다. 다만 CSS width 수정 한 줄만으로는 부족했다.**

### 23.3 Q3 — smart-life / subscription / summary-banner-2 acceptance 후보 전환 타당성

결론: **세 섹션 모두 acceptance 후보로 전환 타당.**

근거:

| section | height delta | remaining issue |
|---|---|---|
| smart-life | 0~1px | image crop, minor spacing |
| subscription | 0px | image not injected (stable baseline) |
| summary-banner-2 | 0~1px | visual tuning only |

space-renewal height delta 17px 대비 이 셋은 구조 misalignment가 없다.
남은 문제는 이미지 crop 위치, 텍스트 색 미세 차이, spacing 1~2px 수준이며 이는 사람 눈으로 acceptance 판단을 내려야 하는 영역이다. Codex가 추가 수정 없이 사용자 visual acceptance 대기 상태로 표시하면 된다.

**Codex 지시:**
1. `smart-life`, `subscription`, `summary-banner-2` 세 섹션을 acceptance backlog에서 `pending → acceptance-ready` 로 표시한다
2. 추가 수정 없이 사용자 visual acceptance 대기 상태로 전환한다

### 23.4 Q4 — diff metric보다 사람 눈으로 봐야 하는 시각 지표

diff mismatch % 수치는 전체 픽셀 비율 기반이므로 아래 문제를 잘 잡지 못한다.

1. **section width vs viewport** — 클론 섹션이 viewport 전폭을 채우는가 아니면 좁게 렌더되는가. space-renewal 사례처럼 28px 차이가 수치로는 작게 보이지만 눈에는 크다.
2. **카드 rhythm** — 카드 간격, 카드 높이의 일관성. reflow가 생기면 카드마다 높이가 달라지고 리듬이 깨진다.
3. **이미지 실제 노출** — `src=""` 또는 `visibility:hidden` 상태인 이미지가 reference/working 중 어느 한쪽에만 있는 경우. diff%는 낮아도 시각으로는 비어보인다.
4. **섹션 간 배경 연속성** — 상하 섹션 배경색이 이어지는가 끊기는가. diff%는 이걸 경계부 수픽셀로만 반영한다.
5. **텍스트 줄바꿈 위치** — 동일 텍스트여도 width 차이로 줄바꿈이 달라지면 레이아웃 감각이 완전히 다르게 보인다.

이 지표들은 스크린샷을 나란히 놓고 사람이 직접 보거나, 특정 row/column alignment를 측정하는 별도 pass가 필요하다.

### 23.5 space-renewal 수정 후 흐름

```
[완료] scoped width fix 적용
  ↓
[완료] sandbox space-renewal 재캡처
  ↓
[결과] height delta: 17px 유지
  ↓
[완료] mixed-card layout 분해
  - first card hero
  - 2/3 cards compact row
  ↓
[완료] item geometry live 수준 정렬
  ↓
[판단] screenshot diff는 reference artifact 영향이 큼
  ↓
[전환] 사용자 visual acceptance + artifact caveat 병행
```

### 23.6 현재 하단 섹션 상태 요약 (2026-04-12 기준)

| section | diff | height delta | 상태 | 다음 단계 |
|---|---|---|---|---|
| smart-life | 6.41% | 0~1px | acceptance-ready | 사용자 visual acceptance |
| subscription | 4.69% | 0px | acceptance-ready | 사용자 visual acceptance |
| summary-banner-2 | 5.13% | 0~1px | acceptance-ready | 사용자 visual acceptance |
| space-renewal | 89.77%* | +17px | manual-review with artifact caveat | geometry aligned, live reference artifact 확인 후 acceptance 판단 |

\* latest diff는 실제 layout 붕괴를 직접 의미하지 않는다. live reference의 hidden placeholder image와 backdrop 차이 때문에 mismatch가 과대 계산될 수 있다.

### 23.7 이전 섹션 기록 오류 정정

`Section 22`에서 다음 두 가지를 잘못 진단했다. 정정 기록을 남긴다.

1. **Section 18 "slot registry 없음" 진단 오류**
   - 실제로 `server.js:6251` `getHomeLowerSlotRegistry()`가 이미 구현되어 있었다.
   - `server.js:6452` `renderHomeLowerSlots()`도 올바른 loop 패턴으로 구현되어 있었다.
   - `llm.js:34` `buildDefaultHomeSlotRegistry()`에도 sources가 열거되어 있었다.
   - Section 18은 코드를 읽기 전에 작성된 섹션이었다. Section 22에서 errata로 정정했다.

2. **Section 17.11 "git not a repository" 진단 오류**
   - 실제로 git이 초기화되어 있었으며 최근 커밋 `0d61dd7`이 존재했다.
   - Section 22에서 정정 완료.

아직 수정되지 않은 버그:

1. **Bug 1 (server.js:6336)** — `options.homeVariant === "custom"` URL 파라미터 bypass
   - brand-showroom render 분기에 `options.homeVariant === "custom" ||` 조건이 남아 있다
   - 이 조건이 있으면 URL query param이 workspace activeSourceId를 무시한다
   - 제거 대상이지만 현재까지 미수정 상태

2. **Bug 2 — figma source void**
   - `llm.js:34`에 `figma-home-*-v1` source가 열거되어 있지만 실제 render 분기가 없다
   - LLM이 figma variant를 선택해도 아무것도 렌더되지 않는다
   - LLM 기능 진입 전 구현 필요

3. **Bug 3 — main-reflected acceptance gap**
   - `main-reflected` 상태는 Codex가 임의로 정의한 것으로 공식 legend에 없다
   - main에 적용된 섹션이 accepted-sandbox 상태가 아닐 수 있다
   - acceptance bundle 전체 통과 전까지 LLM gate는 blocked-by-acceptance 상태

4. **PLP productGrid selector=None (구조적 차단)**
   - `data/normalized/plp-groups/index.json` 8개 항목 전부 `productGrid.selector=None`
   - `applyWorkspacePageVariants()` at `server.js:5193`이 selector 없으면 skip한다
   - PLP의 핵심 컴포넌트에 `data-codex-slot` annotation이 붙지 않음
   - LLM이 category 페이지의 상품 그리드를 편집할 수 없는 상태
   - PLP extractor가 실제 CSS selector를 추출하도록 수정 필요
